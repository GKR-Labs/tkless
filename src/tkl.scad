...
tkl() {
echo("tkl()", "min_h", $min_h);
    if(part == 0) {
        echo("tkl() part==0", "min_h", $min_h);
        spart = [1,1,1,1];
        divided_keyboard(spart[0], spart[1]) keeb_cb();
        if(spart[2])
        color("#eeee77")
        split_base(true, false, inplace=true, $min_h=$min_h) base_cb();
        if(spart[3])
        color("#77ee77")
        render() split_base(false, true, inplace=true, $min_h=$min_h) base_cb();
    }
    if(part == 1) {
